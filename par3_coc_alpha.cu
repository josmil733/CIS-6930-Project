...
__global__ void getDepth(char *input_name, float depth[], const unsigned char* depthData){
	...
	depth[x*ydim+y] = depthData[4*(x*ydim+y)]* meters_cnvt; //coalesced
	}
...
__global__ void coc_alpha(float* caPtr, float* depthMap, int ncols, float ap_diam, float f, float d_focus){
	...
	if(x < xdim && y < ydim){
		...
		*(caPtr+2*(y*xdim+x)+1) = 4/pow(*(caPtr+2*(y*xdim+x) ), 2); */		
		 *(caPtr+2*(x*ydim+y) ) = abs(ncols*pow(WSS*f_f,2)*(d_focus-*(depthMap + x*ydim+y))/(WFF*F*(*(depthMap + x*ydim+y))*(d_focus*WFF - f_f*WSS)));
		*(caPtr+2*(x*ydim+y)+1) = 4/pow(*(caPtr+2*(x*ydim+y) ), 2);
