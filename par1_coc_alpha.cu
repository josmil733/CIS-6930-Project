...
__global__ void getDepth(char *input_name, float depth[], const unsigned char* depthData, int *xPtr, int *yPtr){
	...
	if(x < *xPtr && y < *yPtr){
	depth[ y*(*xPtr)+x ] = depthData[4*(y*(*xPtr)+x)]* meters_cnvt;//not coalesced; would need to switch roles of x and y, or something similar.
	}
...
__global__ void coc_alpha(float* caPtr, int *xPtr, int *yPtr, float* depthMap, int ncols, float ap_diam, float f, float d_focus){
	...
	if(x < *xPtr && y < *yPtr){
		*(caPtr+2*(y*(*xPtr)+x) ) = abs(ncols*pow(WSS*f_f,2)*(d_focus-*(depthMap + y*(*xPtr)+x))/(WFF*F*(*(depthMap + y*(*xPtr)+x))*(d_focus*WFF - f_f*WSS)));
		*(caPtr+2*(y*(*xPtr)+x)+1) = 4/pow(*(caPtr+2*(y*(*xPtr)+x) ), 2);
	}
...
void time(char *input_file, float* depthPtr, const unsigned char* depthData, int* h_xPtr, int* h_yPtr, int* d_xPtr, int* d_yPtr, float* caPtr){
	...
	for(int i=0; i < REPS; i++){
		...
		getDepth<<<dimGrid,dimBlock>>>(input_file, depthPtr, depthData,d_xPtr, d_yPtr);

		coc_alpha<<<dimGrid,dimBlock>>>(caPtr, d_xPtr, d_yPtr, depthPtr, 3, 2.5, 5.5, 4.5);
...
int main(int argc, char *argv[]){
	...
	if(argv[2] != NULL){
		time(argv[1], d_depthPtr, d_depthData, h_xPtr, h_yPtr, d_xPtr, d_yPtr, d_caPtr);
	...
	coc_alpha<<<dimGrid,dimBlock>>>(d_caPtr, d_xPtr, d_yPtr, d_depthPtr, 3, 2.5, 5.5, 4.5);
	cudaDeviceSynchronize();	
